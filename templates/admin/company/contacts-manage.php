...
/** @var CompanyContactDTO|null $edit_contact */
$customer_id = (int) ($edit_contact?->customer_id ?? 0);

$customer_label = null;
if ($editing && $edit_contact) {
    // lấy tên khách hàng theo customer_id
    $customer_svc =  \TMT\CRM\Shared\Container::get('customer-service');
    $customer     = $customer_svc->get_by_id($customer_id);
    $customer_label = $customer?->name ?? ('#' . $customer_id);
}
