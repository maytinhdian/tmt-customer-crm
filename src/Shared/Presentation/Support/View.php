...
final class View
{
    ...
    {
        if (array_key_exists('edit_contact', $vars)) {
            $t = is_object($vars['edit_contact']) ? get_class($vars['edit_contact']) : gettype($vars['edit_contact']);
            error_log('[View] BEFORE include edit_contact type = ' . $t);
        }
